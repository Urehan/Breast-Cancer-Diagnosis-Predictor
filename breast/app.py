...
Engineered features are auto-calculated using estimated formulas since the original training
formulas weren't available — toggle off "Auto-calculate" in the sidebar to enter exact values manually.
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    auto_calc = st.checkbox("Auto-calculate from raw measurements", value=True)
    if not auto_calc:
        st.caption("Exact training formulas unknown — these are estimates. Edit manually below if you have real values.")

    # Estimated formulas (exact training pipeline formulas are unknown).
    # These follow standard tumor-shape feature engineering conventions:
    calc_shape_irregularity = (perimeter_mean ** 2) / area_mean if area_mean else 0.0
    calc_border_complexity = concave_points_mean * perimeter_mean
    calc_tumor_aggressiveness = compactness_mean * concavity_mean * (1 + concave_points_mean)
    calc_radius_texture_interaction = radius_mean * texture_mean
    calc_radius_concavity_interaction = radius_mean * concavity_mean
    calc_concavity_density = concavity_mean / area_mean if area_mean else 0.0
    calc_malignancy_risk_score = (compactness_mean + concavity_mean + concave_points_mean) / 3

    shape_irregularity = st.number_input(
        "Shape irregularity", value=float(calc_shape_irregularity), step=0.01,
        format="%.4f", disabled=auto_calc)
    border_complexity = st.number_input(
        "Border complexity", value=float(calc_border_complexity), step=0.01,
        format="%.4f", disabled=auto_calc)
    tumor_aggressiveness = st.number_input(
        "Tumor aggressiveness", value=float(calc_tumor_aggressiveness), step=0.01,
        format="%.4f", disabled=auto_calc)
    radius_texture_interaction = st.number_input(
        "Radius x texture interaction", value=float(calc_radius_texture_interaction), step=0.1,
        format="%.4f", disabled=auto_calc)
    radius_concavity_interaction = st.number_input(
        "Radius x concavity interaction", value=float(calc_radius_concavity_interaction), step=0.01,
        format="%.4f", disabled=auto_calc)
    concavity_density = st.number_input(
        "Concavity density", value=float(calc_concavity_density), step=0.001,
        format="%.5f", disabled=auto_calc)
    malignancy_risk_score = st.number_input(
        "Malignancy risk score", value=float(calc_malignancy_risk_score), step=0.01,
        format="%.4f", disabled=auto_calc)
